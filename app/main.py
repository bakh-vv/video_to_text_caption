...
def extract_frames_from_video(video_path):
    ...
    representative_frames = buf[::videoFPS, :, :, :] # we can keep more frames per second by decreasing the step (using a fraction of videoFPS)
    del buf
    del cap
...
def predict():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            os.remove(file_path)
            predicted_caption_1, predicted_caption_2, predicted_caption_3 = generate_caption(cnn_features_array)
            return render_template('predict.html', my_prediction_1 = predicted_caption_1, my_prediction_2 = predicted_caption_2, my_prediction_3 = predicted_caption_3)
...
def generate_caption(cnn_features_array):
    ...
    sample_cnn_input_unlim, number_of_frames = get_padded_input_cnn_test(cnn_features_array, tf.constant(maximum_caption_length))

    input_seq = table.lookup(tf.constant(append_bos_token(padding_part_of_caption_sequence(number_of_frames))))
    ...
    predicted_caption_1 = [id_to_word[id_] for id_ in sentences[0][initial_input_seq_length:].numpy().tolist()]
    predicted_caption_1 = ' '.join(predicted_caption_1[:get_showable(predicted_caption_1)])
    predicted_caption_2 = [id_to_word[id_] for id_ in sentences[1][initial_input_seq_length:].numpy().tolist()]
    predicted_caption_2 = ' '.join(predicted_caption_2[:get_showable(predicted_caption_2)])
    predicted_caption_3 = [id_to_word[id_] for id_ in sentences[2][initial_input_seq_length:].numpy().tolist()]
    predicted_caption_3 = ' '.join(predicted_caption_3[:get_showable(predicted_caption_3)])
...
def get_padded_input_cnn_test(cnn_features_array, caption_length_tensor):
    ...
    input_cnn_padded_array =  np.concatenate((cnn_features_array, input_cnn_padding_array))
